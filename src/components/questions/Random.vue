<template>
    <BlockComponent title="随机选人">
        ...
        <p>
            ...
            <Accordion>
                <AccordionTab header="编辑代码">
                    ...
                    <InlineMessage severity="error" v-if="codeError"><div class="code" style="color: #f33">{{ codeError }}</div></InlineMessage>
        ...
        <p class="item">
            随机选人摇人显示时长：
            <Slider v-model="configStore.fn.random.shuffleDuration" style="flex-grow: 1; margin: 0 .6em 0 0.2em;" :min="0" :max="2000"
                :step="100" />
            {{ configStore.fn.random.shuffleDuration }}ms
        </p>
        <p class="item">
            随机选人结果显示时长：
            <Slider v-model="configStore.fn.random.displayDuration" style="flex-grow: 1; margin: 0 .6em 0 0.2em;" :min="1000" :max="5000"
                :step="100" />
...
const ChangeCode = (code: string) => {
    try {
        configStore.fn.random.data = JSON.parse(code);
        codeError.value = "";
    } catch (e: any) {
        console.error(e);
